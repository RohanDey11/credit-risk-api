...
<div class="container">
  ...
  <div class="grid">
    <div class="card">
      ...
      <form id="riskForm">
        <div class="form-group"><label>Age</label><input type="number" name="person_age" value="32" min="18" max="80" required><div class="hint">18-80, 15 returns 422</div></div>
        <div class="form-group"><label>Income ($)</label><input type="number" name="person_income" value="59000" min="0" required></div>
        ...
        <div class="form-group"><label>Employment Length</label><input type="number" name="person_emp_length" value="3" step="0.5" min="0" max="50" required></div>
        ...
        <div class="form-group"><label>Loan Amount ($)</label><input type="number" name="loan_amnt" value="5500" min="0" required></div>
        <div class="form-group"><label>Interest Rate (%)</label><input type="number" name="loan_int_rate" value="12.87" step="0.01" min="0" max="30" required></div>
        ...
        <div class="form-group"><label>Credit History Length</label><input type="number" name="cb_person_cred_hist_length" value="3" min="0" max="50" required></div>
    ...
    <div>
      ...
      <div class="card">
        ...
        <p style="font-size:13px;color:#9aa0b2;margin-bottom:12px">Most freelance code crashes on bad input. This returns 422.</p>
        <div style="background:#0f1115;border:1px solid #2a2f3f;border-radius:8px;padding:12px;font-family:monospace;font-size:12px;color:#9aa0b2">Try Age=15 → 422 Unprocessable Entity</div>
      </div>
    </div>
  </div>
  <div class="footer">Rohan Dey — MSc Data Science | AI Research Intern @ TCG CREST | BS Data Science, IIT Madras<br>FastAPI + Pydantic V2 + Docker + Render + UptimeRobot + pytest</div>
</div>
<script>
document.getElementById('riskForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      resultDiv.innerHTML=`<div class="result-title">⚠️ Validation ${res.status}</div><pre style="white-space:pre-wrap;font-size:11px">${JSON.stringify(json,null,2)}</pre>`;
    } else {
...
# FIX FOR 405: Accept GET, HEAD, POST, OPTIONS on /health so UptimeRobot never gets 405
...
async def predict(request: CreditRequest):
    ...
    try:
        ...
        features = preprocess_input(input_dict, columns)
        if model is None:
            return JSONResponse(status_code=500, content={"error": "Model not loaded"})
        prob = model.predict_proba(features)[0][1] if hasattr(model, 'predict_proba') else float(model.predict(features)[0])
        pred = int(prob > 0.5) if hasattr(model, 'predict_proba') else int(model.predict(features)[0])
        latency = int((time.time() - start) * 1000)
        risk_label = "High Risk - Likely Default" if pred == 1 else "Low Risk - Likely Repay"
        return {"prediction": pred, "risk": risk_label, "probability": float(prob), "default_probability": float(prob), "roc_auc": 0.9214, "model": "Random Forest max_depth 10", "latency_ms": latency}
    except Exception as e:
